...
export default function FilmDao() {
      ...
      db.all(query, (err, rows) => {
        if (err) {
          ...
          if (filters.hasOwnProperty(filter)) {
            ...
            // console.log("FILMS: ", films.filter(filters[filter].filterFunction))
          } // if an invalid filter is specified, all the films are returned.
          else resolve(films);
        }
      });
    });
  ...
  this.updateFilm = (id, film) => {
    return new Promise((resolve, reject) => {
      const query =
        "UPDATE films SET title = ?, isFavorite = ?, rating = ?, watchDate = ? WHERE id = ?";
      const watchDate = film.watchDate
        ? film.watchDate.format("YYYY-MM-DD")
        : null;
      let rating;
      ...
      db.run(
        query,
        [film.title, film.favorite, rating, watchDate, id],
        function (err) {
          if (err) {
            reject(err);
          }
