...
import { createFilm, updateFilm } from "../api/API";
...
const FilmForm = (props) => {
  ...
  const [watchDate, setWatchDate] = useState(
    props.film && props.film.watchDate
      ? dayjs(props.film.watchDate).format("YYYY-MM-DD")
      : ""
  );
  const [rating, setRating] = useState(
    props.film && props.film.rating ? props.film.rating : 0
  );
  ...
  const handleSubmit = (event) => {
    ...
    if (!watchDate) setWatchDate(null);
    ...
    // props.addFilm(film);
      ...
      createFilm(film)
        .then((response) => {
          console.log("response: ", response.validationErrors);
          if (response.validationErrors) {
            setErrors(response.validationErrors);
  ...
  return (
    ...
    >
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control
          className={errors.date ? "wrong-field" : ""}
          type="date"
          value={watchDate}
          onChange={(event) => {
            event.target.value
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control
          className={errors.rating ? "wrong-field" : ""}
          type="number"
          min={0}
          max={5}
          step={1}
      ...
      <Link className="btn btn-danger mx-2 my-3" to={nextpage}>
        Cancel
      </Link>
    </Form>
  );
