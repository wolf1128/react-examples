...
export function FilmListLayout(props) {
  ...
  const filteredFilms =
    filterLabel in props.filters
      ? props.films.filter(props.filters[filterLabel].filterFunction)
      : props.films;
    ...
    <>
      <Row>
        <Col>
          {" "}
          <h1>
            <span id="filter-title">{filterName}</span> films
          </h1>{" "}
        </Col>
      </Row>
...
export function EditLayout(props) {
  ...
  const editableFilm =
    props.films && props.films.find((f) => f.id === Number(filmId));
  ...
  return !editableFilm ? (
    <Row>
      <Col>
        ...
        <Link className="btn btn-primary mx-auto" to="../../" relative="path">
          Go Home!
        </Link>
      </Col>
    </Row>
    ...
    <Row>
      <Col>
        <FilmForm film={editableFilm} editFilm={props.editFilm} />
      </Col>
    </Row>
  );
}
